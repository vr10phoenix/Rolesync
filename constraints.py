...
def _job_hopping_titles_chasing(candidate: dict) -> bool:
    """
    Heuristic for 'title-chasing': 3+ employer switches at <=1.5 years (18mo)
    average tenure, with escalating seniority words in titles.
    """
...
def _recent_langchain_only(candidate: dict, ref: date) -> bool:
    """
    Flags candidates whose only visible AI exposure is LangChain/OpenAI-API
    calls within the last 12 months, with no pre-LLM-era production ML
    evidence (no mention of classic retrieval/ranking/ML-infra terms in
    roles older than 12 months).
    """
...
def _stale_architect_not_coding(candidate: dict, ref: date) -> bool:
    """
    Senior title + no production code signal in the most recent role,
    where that role started 18+ months ago and reads as pure leadership.
    """
    history = candidate.get("career_history", []) or []
    if not history:
        return False
...
def evaluate_candidate(candidate: dict, reference_date: date | None = None) -> ConstraintResult:
    ...
    # Visa/location hard-ish constraint: only disqualify if outside India AND no relocation signal.
    ...
    # --- Soft scoring (only meaningful if not disqualified, but computed regardless for transparency) ---
    breakdown: dict[str, float] = {}

    # 1) Core retrieval/vector-DB production evidence (most important "must-have")
    ...
    # 4) Product company (non-consulting) experience bonus
    breakdown["product_company_experience"] = (0.10 if _has_prior_product_company_experience(candidate) else 0.0)

    # 5) Nice-to-haves: LoRA/PEFT, LTR, HR-tech/marketplace, OSS
    ...
    # 7) Behavioral availability: recency of activity, response rate, notice period
...
if __name__ == "__main__":
    ...
    #     result = evaluate_candidate(c)
        # print("=" * 80)
        # print(result.candidate_id, "DISQUALIFIED" if result.disqualified else "PASSES HARD FILTERS")
        # for r in result.disqualifier_reasons:
        #     print("  -", r)
        # print("  soft_score:", result.soft_score)
        # print("  breakdown:", json.dumps(result.soft_score_breakdown, indent=2))
