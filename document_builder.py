...
class CandidateDocument:
    """Container for everything downstream code needs about one candidate."""
    candidate_id: str
    text: str                     
...
def _build_career_section(career_history: list[dict]) -> str:
    """
    Adding career data in structured secction
    """
    ...
    return "CAREER HISTORY (most recent first):\n" + "\n\n".join(blocks)
...
def _build_signals_section(signals: dict) -> str:
    """
    Renders the behavioral/availability signals as natural language.
    """
    if not signals:
...
def build_candidate_document(candidate: dict) -> CandidateDocument:
    """
    Builds the embeddable text and flat metadata dict used for
    hard filtering
    """
    profile = candidate.get("profile", {}) or {}
    signals = candidate.get("redrob_signals", {}) or {}
