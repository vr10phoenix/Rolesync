...
def cross_encocdder_rerank(query_text: str, query_output: list, top_k: int = 5):
    """
    ...
    print(f"Formatting {len(query_output)} candidates for semantic comparison...")
    pairs = [[query_text, doc.page_content] for doc in query_output]

    print("Reranking process started : ")
    scores = reranker.predict(pairs)

    scored_candidates = list(zip(query_output, scores))
    scored_candidates.sort(key=lambda x: x[1], reverse=True)

    final_shortlist = scored_candidates[:top_k]
    
    print(f"Re-ranking completed : {len(query_output)} down to top {top_k}.")
...
if __name__ == "__main__":
    ...
    top_n_finalists = cross_encoder_rerank(sample_jd_query, query_output=top_N_docs, top_k=5)
